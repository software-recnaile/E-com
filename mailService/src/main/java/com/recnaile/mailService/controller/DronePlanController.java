...
public class DronePlanController {
    ...
    public ResponseEntity<DronePlanDocument> updatePaymentStatus(
        ...
        DronePlanDocument.PaymentStatus paymentStatus =
                DronePlanDocument.PaymentStatus.valueOf(status.toUpperCase());
        DronePlanDocument document = dronePlanService.updatePaymentStatus(referenceId, paymentStatus);
        return document != null ? ResponseEntity.ok(document) : ResponseEntity.notFound().build();
    }
    ...
    public ResponseEntity<DronePlanDocument> updateProcessStatus(
        ...
        DronePlanDocument document = dronePlanService.updateProcessStatus(referenceId, status);
        return document != null ? ResponseEntity.ok(document) : ResponseEntity.notFound().build();
    }
